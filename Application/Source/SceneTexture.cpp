...
void SceneTexture::Init() 
{
	...
	//meshList[GEO_QUAD]->textureID = LoadTGA("Image//muscle_capoo.tga");

	meshList[GEO_QUAD] = MeshBuilder::GenerateCube("cube", blue, 1, 1, 1);
	meshList[GEO_QUAD]->material.kAmbient.Set(0.1f, 0.1f, 0.1f);
	meshList[GEO_QUAD]->material.kDiffuse.Set(0.6f, 0.6f, 0.6f);
	meshList[GEO_QUAD]->material.kSpecular.Set(0.3f, 0.3f, 0.3f);
	meshList[GEO_QUAD]->material.kShininess = 1.f;
...
void SceneTexture::Render() //My Own Pattern
{
	// Render VBO here
	...
	//RenderMesh(meshList[GEO_SPHERE], lights[0].isOn);
	//modelStack.PopMatrix();
}
