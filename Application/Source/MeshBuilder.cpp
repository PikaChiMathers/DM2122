...
Mesh* MeshBuilder::GenerateRevQuad(const std::string& meshName, Color color, float lengthX, float lengthY)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;

	v.pos.Set(0.5f * lengthX, 0.5f * lengthY, 0.f);	 v.normal.Set(0, 0, 1);	v.color = color; v.texCoord.Set(1, 0);	vertex_buffer_data.push_back(v); //v0
	v.pos.Set(-0.5f * lengthX, 0.5f * lengthY, 0.f); v.normal.Set(0, 0, 1);	v.color = color; v.texCoord.Set(0, 0);	vertex_buffer_data.push_back(v); //v1
	v.pos.Set(-0.5f * lengthX, -0.5f * lengthY, 0.f); v.normal.Set(0, 0, 1); v.color = color; v.texCoord.Set(0, 1);	vertex_buffer_data.push_back(v); //v2
	v.pos.Set(0.5f * lengthX, -0.5f * lengthY, 0.f); v.normal.Set(0, 0, 1);	v.color = color; v.texCoord.Set(1, 1);	vertex_buffer_data.push_back(v); //v3
