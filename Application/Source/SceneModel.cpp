...
void SceneModel::Init() 
{
	...
	meshList[GEO_QUAD] = MeshBuilder::GenerateQuad("sidm", Color(.5f, 1, .5f), 1.f, 1.f);
	...
	meshList[GEO_QUAD]->material.kShininess = .5f;
...
void SceneModel::Render() //My Own Pattern
{
	// Render VBO here
	...
	modelStack.Translate(0, 0, 0.009525);
	...
	modelStack.PushMatrix();
	RenderMesh(meshList[GEO_CHAIR], lights[0].isOn);
	modelStack.PopMatrix();

	modelStack.PushMatrix();
	RenderMesh(meshList[GEO_DARTBOARD], lights[0].isOn);
	modelStack.PopMatrix();

	modelStack.PushMatrix();
	RenderMesh(meshList[GEO_DART], lights[0].isOn);
	modelStack.PopMatrix();

	modelStack.PushMatrix();
	RenderMesh(meshList[GEO_DOORMAN], lights[0].isOn);
	modelStack.PopMatrix();

	modelStack.PushMatrix();
	RenderMesh(meshList[GEO_SHOE], lights[0].isOn);
	modelStack.PopMatrix();

	modelStack.PushMatrix();
	RenderMesh(meshList[GEO_BOTTLE], lights[0].isOn);
	modelStack.PopMatrix();
